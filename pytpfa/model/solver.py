...
class TPFASolver:
    ...
    def preprocess_problem_data(self, reservoir_path):
        ...
        reservoir_length = (config.input.lx, config.input.ly, config.input.lz)
        reservoir_h = tuple([reservoir_length[i] / reservoir_sizes[i] for i in range(3)])
        ...
        for well_name, well in config.wells.items():
            well_coordinates = (well.block_coord_x, well.block_coord_y, well.block_coord_z)
            well_index = (
                np.floor(well_coordinates[0] / self.hx).astype(int),
                np.floor(well_coordinates[1] / self.hy).astype(int),
                np.floor(well_coordinates[2] / self.hz).astype(int),
            )
            for coord, size in zip(well_index, reservoir_sizes):
                if not (0 <= coord < size):
                    raise ValueError(
                        f"Well {well_name} coordinates {well_index} are out of bounds for reservoir size {reservoir_sizes}"
                    )
