...
usage() {
  ...
  echo "Usage: $0 --petsc_dir=<PATH> --mode=<OPT|DEBUG> [--stage=<NUMBER>]"
  echo "Example: $0 --petsc_dir=/home/user/petsc --mode=OPT --stage=3"
  exit 1
}
...
if [[ -z "$PETSC_DIR_ARG" || -z "$MODE_ARG" ]]; then
  usage
fi
...
echo "========================================================"
echo "Starting PETSc build in ${MODE} mode."
...
run_step 3 "Building PETSc libraries"               "${LOG_DIR}/03-build_all.log"             make all
run_step 4 "Installing PETSc to staging dir"        "${LOG_DIR}/04-install_petsc.log"         make DESTDIR="${PETSC_DIR}/${INSTALL_SUBDIR}" install
run_step 5 "Building petsc4py"                      "${LOG_DIR}/05-pip_build.log"             bash -c "pip uninstall -y petsc4py && pip install ."
